...
### Fixed (수정)
- **이미 해결된 에러 재업로드 문제 해결**
  - markResolved가 이미 resolved 상태인 에러를 null 대신 레코드 반환
  - 이미 해결된 에어에 대해서도 resolution 업데이트 허용
  - fixhive_resolve가 "Error with ID not found"를 반환하지 않도록 수정
  - 사용자가 해결된 에러를 커뮤니티에 다시 공유 가능
...
### Fixed (수정)
- **fixhive_list ID 표시 개선**
  - 전체 UUID를 백틱(`)으로 감싸서 표시
  - 복사 가능한 형식으로 ID 제공
  - fixhive_resolve 사용 시 ID 복사 용이
- **클라우드 업로드 디버깅 로그 추가**
  - uploadResolution에 상세 로그 추가 (임베딩 생성, 중복 확인, 삽입 과정)
  - fixhive_resolve에 업로드 진행 상태 로그 추가
  - 업로드 실패 시 상세 에러 정보 출력
...
### Fixed (수정)
- **에러 로깅 진단 기능 추가**
  - 에러 감지 시 confidence, type, tool, message 로그 추가
  - confidence < 50%일 경우 저장되지 않음을 로깅
  - sessionId 누락 시 경고 및 안내 메시지 추가
  - 에러 저장 성공 시 레코드 ID 로그 출력
  - 에러가 기록되지 않는 문제 디버깅 가능
